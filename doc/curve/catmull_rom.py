...
def parameterization(pt_w, ctrl_pts_np, tau=0.5, N=4):
    ...
    v1 = pt2[:3] - pt1[:3]
    v2 = pt_w[:3] - pt1[:3]
    ratio = np.dot(v1, v2.T) / np.sum(
        v1**2
    )  # 除以v1的模，再除以v1的模得到0~1之间的比例
...
class CentripetalCatmullRomSpline:
    ...
    def get_points_part(self, four_ctrl_pts, n_points):
        ...
        t = t.reshape(-1, 1)
        A1 = (t1 - t) / (t1 - t0) * four_ctrl_pts[0] + (t - t0) / (
            t1 - t0
        ) * four_ctrl_pts[1]
        A2 = (t2 - t) / (t2 - t1) * four_ctrl_pts[1] + (t - t1) / (
            t2 - t1
        ) * four_ctrl_pts[2]
        A3 = (t3 - t) / (t3 - t2) * four_ctrl_pts[2] + (t - t2) / (
            t3 - t2
        ) * four_ctrl_pts[3]
