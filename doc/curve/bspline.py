...
class CubicBSplineCurve:
    ...
    def plot(self, num=10, raw_points=None, sphere_size=0.1):
        ...
        ctrlpts_sphere = pointcloud_to_spheres(
            self.ctrlpts, sphere_size=sphere_size, color=[0.7, 0.3, 0.3]
        )
    ...
    def get_points(self, knots, return_knots=False):
        ...
        M_b = (
            np.asarray([[-1, 3, -3, 1], [3, -6, 3, 0], [-3, 0, 3, 0], [1, 4, 1, 0]]) / 6
        )
...
class BSplineCurve:
    def __init__(self, ctrlpts, degree=3, type="uniform"):
        ...
        if self.type == "uniform":
            ...
            self.knots = (
                [0] * (degree + 1)
                + [i for i in range(1, ctrlpts.shape[0] - degree)]
                + [ctrlpts.shape[0] - degree] * (degree + 1)
            )
        elif self.type == "nonuniform":
            self.knots = np.asarray([0, 1])
            self.knots = np.hstack(
                [self.knots, np.random.uniform(0.01, 0.99, self.num_knots - 2)]
            )
    ...
    def print_infos(self):
        ...
        for idx, u in enumerate(mean_knots):
            ...
            if self.knots[idx] != self.knots[idx + 1]:
                new_u = np.asarray(
                    [(u - self.knots[idx]) / (self.knots[idx + 1] - self.knots[idx])]
                ).reshape(-1, 1)
                ...
                print(
                    ...
                    ", weight: ",
                    weight,
                    ", sum: {:.3f}".format(np.sum(weight)),
                    ", coeff: ",
                    coeff,
                    ", sum: {:.3f}".format(np.sum(coeff)),
                )
            else:
                print(
                    "u: ",
